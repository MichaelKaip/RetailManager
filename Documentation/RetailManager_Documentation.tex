...
\subsection{Adding a "taxable" flag}
\boxed{\includegraphics[scale=0.41]{pic130}}\\\\
Alternatively there can be also set flags for different tax rates.\\\\
\boxed{\includegraphics[scale=0.48]{pic131}}\\\\
\boxed{\includegraphics[scale=0.48]{pic132}}\\\\
\boxed{\includegraphics[scale=0.48]{pic133}}

\subsubsection{Adding the tax rate to App.Config}
\boxed{\includegraphics[scale=0.48]{pic134}}

\subsubsection{Adding a ConfigHelper.cs}
\boxed{\includegraphics[scale=0.48]{pic135}}\\\\
Now it has to be added to the container in \boxed{Bootstrapper.cs} to enable DI:\\\\
\boxed{\includegraphics[scale=0.48]{pic136}}\\\\
And finally it can be requested from \boxed{SalesViewModel.cs} in order to gain access from there to it:\\\\
\boxed{\includegraphics[scale=0.48]{pic137}}\\\\
\boxed{\includegraphics[scale=0.48]{pic138}}\\\\
Now the TAX and the SucTotal getters can be implemented:\\\\
\boxed{\includegraphics[scale=0.48]{pic139}}\\\\
And the getter for TOTAL :\\\\
\boxed{\includegraphics[scale=0.48]{pic140}}\\\\
And finally updating the NotifyOfPropertyChange for SubTotal, Tax and Total in \textbf{AddToCart() and RemoveFromCart()}:\\\\
\boxed{\includegraphics[scale=0.48]{pic141}}
\newpage

\section{Refactorings}

\subsection{Adding foreign keys to the database tables}
\begin{enumerate}
\item Inventory.sql\\\\
\boxed{\includegraphics[scale=0.48]{pic142}}
\item Sales.sql\\\\
\boxed{\includegraphics[scale=0.48]{pic143}}
\item SaleDetail.sql\\\\
\boxed{\includegraphics[scale=0.48]{pic144}}
\end{enumerate}

\subsection{Change the naming related to the Entity Framework database connection}
\begin{enumerate}
\item RMDataManager.WebConfig $\to$ DefaultConnection\\\\
From "DefaultConnection" to \textbf{"EntityFrameworkData"}\\\\
\boxed{\includegraphics[scale=0.3]{pic145}}\newpage
\item RMDataManager.Models.IdentityModels.cs\\\\
\boxed{\includegraphics[scale=0.38]{pic146}}
\item Migration\\\\
\boxed{\includegraphics[scale=0.38]{pic147}}
\item To silence this error first delete the generated class RMDataManager.Migrations.Configuration.cs
\item Using SWAGGER to re-create the user in the "new" database\\\\
\boxed{\includegraphics[scale=0.48]{pic148}}
\item Refreshing database in SQL Server-Object-Explorer will complete the re-naming
\end{enumerate}

\subsection{Using IoC Inversion Control (Caliburn Micro) in ShellViewModel.cs}
\boxed{\includegraphics[scale=0.48]{pic149}}

\subsection{Improving the SalesView by displaying additional product information}
\boxed{\includegraphics[scale=0.48]{pic150}}
\newpage

\section{Posting sales data back to the API}
The aim of doing this is to save a sale into the databse in order to do the checkout.
\begin{enumerate}
\item Adding the taxrate to Web.Config (API)\\\\
\boxed{\includegraphics[scale=0.48]{pic151}}
\item Adding a ConfigHelper.cs to \boxed{RMDataManager.Library.Helpers} to pull the tax rate\\\\
\boxed{\includegraphics[scale=0.415]{pic152}}
\item Sending the CartItemModel over to the API\\\\
But only non-critical information (ID and Quantity) should be sent in order to not trust on the API relating to critical information like RetailPrice, SubTotal and Total - because this information could have been manipulated manually.\\\\
Critical data has to be fetched from the database and re-calculated.
\begin{enumerate}
\item Creating a SaleModel.cs and SaleDetailModel.cs \textbf{(from the Frontend-side)} to define the data which is going to post to the API\\\\
\boxed{\includegraphics[scale=0.415]{pic153}}\\\\
\boxed{\includegraphics[scale=0.415]{pic154}}
\item Now CanCheckOut() and CheckOut() in the SalesViewModel can be implemented\\\\
\boxed{\includegraphics[scale=0.415]{pic155}}\\\\
Don't forget adding NotifyOfPropertyChange $\to$ CanCheckout to "RemoveFromCart" and "AddToCart"...\\\\
\textbf{Checkout-Button is active now!!!}\\\\
\item Creating a new API-Controller in \boxed{RMDataManager.Controllers}\\\\
\boxed{\includegraphics[scale=0.415]{pic156}}\\\\
\item Creating a SaleModel.cs and SaleDetailModel.cs \textbf{(from the Backend-side)} to receive the data from Frontend \\\\
\boxed{\includegraphics[scale=0.415]{pic157}}\\\\
\boxed{\includegraphics[scale=0.415]{pic158}}\\\\
\item Creating a SaleEndPoint.cs\\\\
\boxed{\includegraphics[scale=0.415]{pic159}}\\\\
\item Creating an Interface and adding it to the Bootstrapper for dependency injection\\\\
\boxed{\includegraphics[scale=0.415]{pic160}}\\\\
\item Adding the SaleEndPoint to the constructor of the SalesViewModel and creating a private backing field, which is gonna be initialized from the constructor\\\\
\boxed{\includegraphics[scale=0.415]{pic161}}\newpage
\item Implementig the SaleController in order to handle the posted data from the Frontend\\\\
\boxed{\includegraphics[scale=0.415]{pic162}}
\begin{enumerate}
\item Creating a class SaleData.cs in \boxed{RMDataManager.Library.DataAccess}\\\\
\boxed{\includegraphics[scale=0.415]{pic163}}\\\\
\boxed{\includegraphics[scale=0.415]{pic164}}\newpage
\begin{enumerate}
\item Implementing a Method in \boxed{RMDataManager.Library.DataAccess.ProductData} and a stored procedure for getting the information about products from the database\\\\
\boxed{\includegraphics[scale=0.415]{pic165}}\\\\
\boxed{\includegraphics[scale=0.415]{pic166}}
\item Creating a class ConfigHelper.cs in \boxed{RMDataManager.Library} to get the taxrate\\\\
\boxed{\includegraphics[scale=0.415]{pic172}}\\\\
It's reading the taxrate from \textbf{Web.Config}.
\item Creating a new stored procedure \boxed{spSale\_Insert}\\\\
\boxed{\includegraphics[scale=0.415]{pic167}}
\item Creating a new stored procedure \boxed{spSaleDetail\_Insert}\\\\
\boxed{\includegraphics[scale=0.415]{pic168}}
\end{enumerate}\newpage
\item Creating a SaleDetailDBModel in \boxed{RMDataManager.Library.Models}\\\\
\boxed{\includegraphics[scale=0.415]{pic169}}
\item Creating a SaleDBModel in \boxed{RMDataManager.Library.Models}\\\\
\boxed{\includegraphics[scale=0.415]{pic170}}\newpage
\item Creating a stored procedure \boxed{spSale\_Lookup} for getting the sale id from the table\\\\
\boxed{\includegraphics[scale=0.415]{pic171}}
\end{enumerate}
\end{enumerate}
\end{enumerate}
